...
<template>
  <section
    ref="sectionRef"
    class="relative bg-accent-50/40 overflow-x-clip min-h-[calc(100vh-310px)]"
  ...
  >
    ...
    <div class="relative mx-auto max-w-[1536px] px-4 sm:px-6 lg:px-12 py-12 lg:py-0 min-h-[calc(100vh-310px)] flex items-center">
      <div class="grid lg:grid-cols-2 gap-12 lg:gap-8 items-center w-full">
        ...
        <div class="relative z-10">
          ...
          <div v-reveal="{ delay: 300 }" class="flex flex-col sm:flex-row gap-4 mt-8">
            <NuxtLink
              to="/#contact"
            ...
            <a
              ...
              class="inline-flex items-center justify-center gap-3 px-7 py-3.5 border-2 border-navy-200 hover:border-accent-400 rounded-full text-sm font-bold text-navy-700 transition-all"
            >
          ...
          <!-- Trust indicators -->
          <div v-reveal="{ delay: 400 }" class="flex items-center gap-6 mt-10 pt-8 border-t border-gray-100">
      ...
      <div
        ...
        class="hidden lg:flex absolute left-[55%] top-[25%] bg-white rounded-xl shadow-xl shadow-navy-900/10 p-4 items-center gap-3 z-10 transition-transform duration-300 ease-out"
        :style="{ transform: cardTransform(15) }"
      >
        <div class="w-12 h-12 rounded-full bg-accent-50 flex items-center justify-center">
          <HeartHandshake class="h-6 w-6 text-accent-500" />
        </div>
        <div>
          <p class="text-lg font-heading font-bold text-navy-900">{{ t('hero.nowOpen') }}</p>
          <p class="text-xs text-navy-500">{{ t('hero.acceptingReferrals') }}</p>
        </div>
      </div>
      ...
      >
        <div class="w-12 h-12 rounded-full bg-navy-700 flex items-center justify-center">
    ...
    <div
      ...
      class="hidden lg:flex absolute right-16 top-[15%] bg-white rounded-xl shadow-xl shadow-navy-900/10 p-4 items-center gap-3 z-10 transition-transform duration-300 ease-out"
      :style="{ transform: cardTransform(25) }"
    >
      <div class="w-12 h-12 rounded-full bg-accent-400/10 flex items-center justify-center">
        <Video class="h-6 w-6 text-accent-500" />
      </div>
      <div>
        <p class="text-lg font-heading font-bold text-navy-900">{{ t('hero.telehealth') }}</p>
        <p class="text-xs text-navy-500">{{ t('hero.telehealthSub') }}</p>
      </div>
    </div>
  </section>
</template>
